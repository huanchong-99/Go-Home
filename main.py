...
{accommodation_section}"""
    ...
    def clear_browser_cookies(self):
        ...
        if os.path.exists(browser_data_dir):
            try:
                self.log_message(f"正在清除浏览器cookie: {browser_data_dir}")

                # 【关键修复】先杀掉所有 Chrome/Edge 进程，释放文件锁
                self.log_message("正在关闭所有浏览器进程以释放文件锁...")
                try:
                    # 尝试杀掉 Chrome 进程
                    ...
                    self.log_message("✅ 浏览器进程已关闭")
                except Exception as kill_error:
                    self.log_message(f"⚠️ 关闭浏览器进程时出现错误（可忽略）: {kill_error}")

                # 现在可以安全删除目录
                shutil.rmtree(browser_data_dir)
                time.sleep(0.5)  # 等待文件系统同步
                self.log_message("✅ 浏览器cookie已清除")
                return True
            except Exception as e:
                self.log_message(f"❌ 清除cookie失败: {e}")
                self.log_message("💡 提示: 请手动关闭所有浏览器窗口后重试")
                return False
        else:
            self.log_message("ℹ️ 未找到browser_data目录，跳过清除")
            return True
    ...
    def start_all_services(self):
        ...
        response = messagebox.askyesno(
            "清除Cookie",
            "是否清除旧的浏览器Cookie?\n\n"
            "如果上次查询出现验证码问题或所有查询都超时，\n"
            "建议清除cookie重新验证。\n\n"
            "⚠️ 清除后首次查询可能需要重新完成验证码。"
        )
