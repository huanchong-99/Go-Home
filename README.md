...
这是一个经典的图论搜索问题。官方平台采用**"预计算 + 启发式搜索"**的混合策略：

#### 核心技术方案
1. **图模型**：将所有站点看作节点，航线/车次作为带权重的边
2. **剪枝策略**：
   - **地理围栏**：只在合理的椭圆范围内搜索（如北京→广州不会考虑哈尔滨中转）
   - **枢纽优先**：优先选择航空公司基地、铁路枢纽城市
   - **连通度过滤**：只考虑线路密集的城市
3. **预计算**：提前计算热门路线的 Top N 方案并缓存
4. **动态规划**：找可达站点的交集，过滤不合理的中转时间
...
| 方面 | 官方平台 | Go-home |
|------|---------|---------|
| **查询范围** | 单一平台（机票或火车） | 跨平台组合（机票+火车） |
| **中转策略** | 基于商业利益和热度 | 纯粹优化用户成本 |
| **空铁联运** | 极少支持（如东方航空APP） | 全面支持 11 个空铁联运枢纽 |
| **国际覆盖** | 机票平台覆盖全球，铁路仅国内 | 统一支持国内 + 152 个国际机场 |
| **透明度** | 算法黑盒，可能受商业影响 | 开源策略，用户可控 |
| **定制化** | 固定算法 | 可通过配置文件调整策略 |

### Go-home 的优势

1. **真正的多模式组合**：不只是找中转，而是智能组合不同交通工具
2. **跨平台比价**：在一个界面比较飞机+火车的组合方案
3. **国际中转支持**：自动识别最佳国际中转枢纽
4. **程序化计算**：价格、时长 100% 准确，无商业偏见
5. **灵活可配置**：用户可调整中转时间、住宿费用等参数
...
- 中转枢纽数量对应预计时间：8个≈10-15分钟，20个≈20-40分钟，44个≈50-90分钟
