...
- 如果已安装 **Chrome**，程序将优先使用 Chrome
- 如果只有 **Edge**（Windows 10/11 自带），程序会自动使用 Edge
- 两者都支持，无需额外配置
...
| 模式                   | 说明                                   | 适用场景                           |
| ---------------------- | -------------------------------------- | ---------------------------------- |
| **智能中转模式** | 程序主动遍历 44 个枢纽城市查询中转方案 | 复杂行程、无直达线路、追求最低价格 |
| **标准模式**     | 让 AI 自行决定查询策略                 | 简单行程、有直达线路               |

> 📊 **效果对比**：两种模式的实际查询效果对比请查看 [比较.md](比较.md)，文档末尾有 AI 总结的两种模式显著区别。
>
> **结论**：智能中转模式能找到更便宜的方案（实测可省约 200 元），路线逻辑更符合地理现实。
