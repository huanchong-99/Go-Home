...
- **国际节点智能开关**：
  - **默认关闭**：国内↔国外路线仅使用国内枢纽中转，适合短距离跨国（如东南亚、东亚）
  - **开启后**：启用全球枢纽网络（含中东、欧洲等国际枢纽），适合跨大洲旅行（如去欧美、非洲）
  - **智能建议**：普通跨国旅行（亚洲内）保持关闭即可，避免不必要的绕路；远程洲际旅行才需开启
...
- **导出全量数据**：现在导出所有原始查询数据，而不是仅导出AI推荐结果
  - ✅ 第一部分：所有单段查询结果（包括成功和失败的查询）
  - ✅ 第二部分：所有组合路线方案（直达方案 + 所有中转方案）
  - ✅ 详细信息：每个查询的ID、路线、交通方式、查询状态、耗时
  - ✅ 完整数据：所有航班/车次的原始数据，未经筛选
- **智能分组**：导出数据按段数分组（直达、中转），按中转城市分类
- **文件命名优化**：自动命名为"Go-home_完整数据_出发地到目的地_日期_时间戳.txt"
...
- **完整详情输出**：强制要求AI为所有推荐方案都输出完整的航班号/车次号
- **统一格式**：每个推荐方案（最便宜、最快、性价比最高）都必须包含：
  - ✅ 具体的航班号或车次号
  - ✅ 出发时间、到达时间
  - ✅ 价格详情
  - ✅ 中转等待时间（如有）
- **示例格式**：在AI提示词中提供清晰的输出示例，确保一致性
- **用户友好**：所有推荐方案的信息一目了然，便于用户对比选择
...
A: **国际节点开关默认关闭**，适合大多数跨国旅行场景。只有在特定情况下才需要开启：

**❌ 不需要开启的场景（保持默认关闭）**：
- 国内↔东南亚（如北京→曼谷、上海→新加坡）
- 国内↔东亚（如北京→东京、上海→首尔）
- 国内↔港澳台（如广州→香港、上海→台北）
- 短距离跨国（飞行时间 < 6小时）

**✅ 需要开启的场景**：
- 跨大洲旅行（如北京→纽约、上海→伦敦、广州→悉尼）
- 远程洲际航线（飞行时间 > 8小时）
- 需要考虑中东枢纽中转的路线（如迪拜、多哈转机）
- 需要考虑欧洲枢纽中转的路线（如法兰克福、阿姆斯特丹转机）

**为什么默认关闭？**
- 短距离跨国使用国内枢纽（如广州、昆明）直飞或中转即可，效率最高
- 开启国际节点会增加大量国际中转枢纽（如迪拜、多哈），对于短途反而绕路
- 查询时间会显著增加（多查询 30+ 个国际枢纽）
- 价格可能反而更贵（国际中转成本高于国内直飞）
